...
from papers.models import Paper, Embedding, EmbeddingReduced
            ...
            for paper, embedding in zip(batch, embeddings):
                ...
                embedding_reduced_objects.append(
                    EmbeddingReduced(
                        ...
                        model_name=model_name,
                        embedding_type="abstract",
                        vector=embedding_512.tolist(),
                    )
                )

            Embedding.objects.bulk_create(embedding_objects, ignore_conflicts=True)
            EmbeddingReduced.objects.bulk_create(embedding_reduced_objects, ignore_conflicts=True)
