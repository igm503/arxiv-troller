...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        papers_queryset = Paper.objects.filter(embeddingvoyage3bit2048__isnull=True).order_by("id")
        ...
        try:
            ...
            for paper, embedding in zip(batch, embeddings):
                embedding_2048_objects.append(
                    EmbeddingVoyage3Half2048(paper=paper, vector=embedding)
                )
                ...
                embedding_256_objects.append(
                    EmbeddingVoyage3Half256(paper=paper, vector=embedding_256)
                )
                embedding_bit = "".join("1" if x > 0 else "0" for x in embedding)
                embedding_bit2048_objects.append(
                    EmbeddingVoyage3Bit2048(paper=paper, vector=embedding_bit)
                )

            EmbeddingVoyage3Half2048.objects.bulk_create(embedding_2048_objects)
            EmbeddingVoyage3Half256.objects.bulk_create(embedding_256_objects)
            EmbeddingVoyage3Bit2048.objects.bulk_create(embedding_bit2048_objects)
